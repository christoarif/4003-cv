...
[a,b] = freqspace(256,'meshgrid'); %reference https://www.mathworks.com/matlabcentral/fileexchange/53195-2d-low-pass-filter-mask
d = zeros(256,256);
for i = 1:256
    for j = 1:256
...
%%
% // Now filter
F_lowpassfilter = F .* ifftshift(H); 
S = abs(F_lowpassfilter).^2;
imagesc(ifftshift(S.^0.1));
colormap('default');
%%
inverse_F = uint8(ifft2(F_lowpassfilter));
imshow(inverse_F); % did not work very well...

%% Now try other approach; blocking out the straight lines perpedincular to the peaks in the spectrum
% actual locations of peaks : (9,241), (249,17)
F(:, 9) = 0;
F(241, :) = 0;
F(17,:) = 0;
...
inverse_F = uint8(ifft2(F));
imshow(inverse_F); %looks much better now!

%% lastly, contrast stretching. 
inverse_F = imsubtract(inverse_F, double(min(inverse_F(:))));
inverse_F2 = immultiply(inverse_F, (255 ./ double(max(inverse_F(:))-min(inverse_F(:)))));

%check min and max pixel value 
min_inverse_F2 = min(inverse_F2(:)); %should be 0 
max_inverse_F2 = max(inverse_F2(:)); %should be 255
imshow(inverse_F2);
...
imagesc(ifftshift(S_primate.^0.01));
colormap('default');
...
%% 2.6e) Display image
imshow(P2)
%% 2.6f) Identify Pink area
imshow(book_img)
%%
[X Y] = ginput(4);
%%
x_desired = [0 0 20 20]; % 1 pixel to 1 mm of the dimension, as stated in the qn
y_desired = [0 20 20 0]; %guesstimate about 20pixels for the pink area

%% set up matrices required to estimate the projective transfomration

v=[0;0;0;20;20;20;20;0] ;
...
P2 = imtransform(book_img, T, 'XData', [0 20], 'YData', [20 0]);
